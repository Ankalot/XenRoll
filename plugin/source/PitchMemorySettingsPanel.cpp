...
PitchMemorySettingsPanel::PitchMemorySettingsPanel(Parameters *params,
    ...
    juce::String algoDescrText = 
        ...
        "       Introduction:\n"
        + bullet + " Each note has harmonicity value (HV) in range [-1; +1] which is shown with "
        "color gradient {red; yellow; green}\n"
        + bullet + " Each note leaves a trace in pitch memory (measure of tonality), it has trace "
        "value (TV) in range [0; +1] which is shown with color gradient {black; white}.\n"
        + bullet + " HV and TV of the note are derived from interractions between that note and "
        ...
        "Let's call it " + DV_ij + ".\n";
    algoDescrText += "   2.2 " + HV_i + " = -" + SUM_j + "(" + TV_j + "*" + DV_ij + ")/" + SUM_j + "(" + TV_j + ")\n";
    algoDescrText += "   2.3 " + TV_i + " = { (" + HV_i + " + 1)*" + TV_0 + ";    if " + HV_i + " <= 0\n";
    algoDescrText += "                      { (1-" + TV_0 + ")*" + HV_i + " + " + TV_0 + ";  if " + HV_i + " > 0\n";
    algoDescrText += "          if " + TV_i + " < " + TV_MIN + ":  " + TV_i + " := 0\n";
    algoDescrText += "   2.4 For each existing trace (let's consider j-th trace):\n";
    algoDescrText += "          " + TV_j + " = min(1.0, " + TV_j + "*2^(min(1.0, " + TV_i + " + " + TV_ADD + ")*" + HV_i + "))\n";
...
void PitchMemorySettingsPanel::paint(juce::Graphics &g) { 
    ...
    g.drawLine(padding, padding * 4 + rowHeight * 3 + Theme::wider, getWidth() - padding, 
        padding * 4 + rowHeight * 3 + Theme::wider, Theme::wider);
}
} // namespace audio_plugin