...
juce::Path MainPanel::getNotePath(const Note &note) {
    ...
    const float w = bar_width_px * note.duration; // width
    const float x1 = note.time * bar_width_px;
    const float x2 = x1 + w;
    const float y1 =
        (params->num_octaves - note.octave - float(note.cents) / 1200) * octave_height_px;
    float dy = float(note.bend) / 1200 * octave_height_px;
    const float y2 = y1 - dy;

    // [0.0f; 1.0f]
    const float roundingCoef =
        static_cast<float>(GlobalSettings::getInstance().getNoteRectRounding());
    const float r = roundingCoef * juce::jmin(h, w) * 0.5f;
